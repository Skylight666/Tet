...
void TetrixBoard::paintEvent(QPaintEvent* event)
{
    ...
    {
        painter.drawLine(squareWidth() * i, 0, squareWidth() * i, squareHeight()*22);
    }

    for (int i = 0; i <= BoardHeight; ++i)
    {
        painter.drawLine(0, (squareHeight() - 0) * i, squareWidth() * 11, (squareHeight() - 0) * i);
    }
    ...
    for (int i = 0; i < BoardHeight; ++i) {
        for (int j = 0; j < BoardWidth; ++j) {
            ...
            QColor color = colorAt(j, BoardHeight - i - 1); 
            if (shape != NoShape)
                drawSquare(painter, rect.left() + j * squareWidth(),
                    boardTop + i * squareHeight(), color);
        }
    }
...
void TetrixBoard::removeFullLines()
{
    ...
    for (int i = BoardHeight - 1; i >= 0; --i) {
        ...
        for (int j = 0; j < BoardWidth; ++j) 
        {
            ...
            {
                if (prevColor == 0x111111)
                {
                    prevColor = colorAt(j, i);
                }

                if (prevColor == colorAt(j, i))
                {
                    dieLine += 1;
                    if (dieLine >= 5)
                    {
